...
namespace Radar
{
    ...
    public sealed class RadarSettings : IPSettings
    {
        ...
        public bool ShowPathsToPOI = true;
        public bool DrawOnlyNearestPOIPath = true;   // else draw all POI paths
        public int PathMaxNodes = 4096;             // safety cap for A*
        public int PathSampleStep = 2;              // draw every Nth node to reduce overdraw
        public Vector4 PathColor = new(1f, 0.6f, 0f, 0.95f); // orange
        public float PathThickness = 3.0f;

        // Add near your existing path settings
        public bool UseDistinctPathColors = true;     // per-POI path colors
        public float DistinctPathSaturation = 0.85f;  // 0..1
        public float DistinctPathValue = 1.00f;       // 0..1
